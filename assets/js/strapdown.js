    ...
    Lexer.prototype.token = function (src, top) {
        ...
        while (src) {
            ...
            if (cap = this.rules.list.exec(src)) {
                src = src.substring(cap[0].length);
                this.tokens.push({type: 'list_start', ordered: isFinite(cap[2])});
...
(function () {
    ...
    function combinePrefixPatterns(regexs) {
        ...
        var escapeCharToCodeUnit = {'b': 8, 't': 9, 'n': 0xa, 'v': 0xb, 'f': 0xc, 'r': 0xd};
    ...
    function extractSourceSpans(node, isPreformatted) {
        ...
        walk(node);
        return {sourceCode: chunks.join('').replace(/\n$/, ''), spans: spans};
    }
    ...
    function appendDecorations(basePos, sourceCode, langHandler, out) {
        ...
        var job = {sourceCode: sourceCode, basePos: basePos};
        langHandler(job);
    ...
    function sourceDecorator(options) {
        ...
        fallthroughStylePatterns.push([PR_LITERAL, /^@[a-z_$][a-z_$@0-9]*/i, null], [PR_TYPE, /^(?:[@_]?[A-Z]+[a-z][A-Za-z_$@0-9]*|\w+_t\b)/, null], [PR_PLAIN, /^[a-z_$][a-z_$@0-9]*/i, null], [PR_LITERAL, new RegExp('^(?:'
            ...
            + '(?:e[+\\-]?\\d+)?'
            + ')'
            + '[a-z]*', 'i'), null, '0123456789'], [PR_PLAIN, /^\\[\s\S]?/, null], [PR_PUNCTUATION, punctuation, null]);
    ...
    function prettyPrintOne(sourceCodeHtml, opt_langExtension, opt_numberLines) {
        ...
        var job = {langExtension: opt_langExtension, numberLines: opt_numberLines, sourceNode: container, pre: 1};
        applyDecorator(job);
...
var markdown = function ($src, $desc) {
    ...
    var markdownEl = document.getElementById($src);
    var htmlEl = document.getElementById($desc);
